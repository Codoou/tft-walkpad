...
def render_sidebar_with_panel(sidebar_bgr: np.ndarray, panel_lines: list[str]) -> np.ndarray:
    ...
    for i, s in enumerate(panel_lines[:8]):  # keep it tidy
        cv2.putText(out, s, (x, y + i * line_h), font, 0.78, (240, 240, 240), 2, cv2.LINE_AA)
        ...
        while True:
            ...
            if DEBUG_PREVIEW:
                ...
                lines = [
                    ...
                    f"Treadmill  now={treadmill.state.speed_kmh:.1f} km/h  {treadmill.state.incline_pct:.1f}%",
                    f"Target    {target_speed:.1f} km/h  {target_incline:.1f}%   sent={sent}",
                    f"History(last {len(hist_tail)}): {hist_tail}",
                ]
